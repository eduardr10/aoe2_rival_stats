    ...
    <script>
        ...
        function create_socket(handler_name) {
            ...
            socket.onmessage = async (event) => {
                ...
                // Solo solicitar análisis si matchId existe, no ha sido solicitado y leaderboard es rm_1v1
                if (!msg.data || !msg.data.matchId || analyzedMatches.has(msg.data.matchId) || msg.data.leaderboardId !== 'rm_1v1') {
                    console.log('Skipping analysis for matchId:', msg.data ? msg.data.matchId : undefined);
                    return;
                }
                analyzedMatches.add(msg.data.matchId);
                // Recarga la página con el matchId en la query
                window.location.replace(`/${player_id}?matchId=${msg.data.matchId}&t=${Date.now()}`);
            };
