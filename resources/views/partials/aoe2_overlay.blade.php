    ...
    <script>
        ...
        function create_socket(handler_name) {
            ...
            socket.onmessage = async (event) => {
                ...
                if (!msg.matchId || analyzedMatches.has(msg.data.matchId) || msg.leaderboardId !== 'rm_1v1') {
                    console.log('Skipping analysis for matchId:', msg.data.matchId);
                    return;
                }
                analyzedMatches.add(msg.matchId);
                // Reload page with matchId in query string to trigger overlay render
                window.location.replace(`/${player_id}?matchId=${msg.matchId}&t=${Date.now()}`);
            };
