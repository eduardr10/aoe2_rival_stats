...
{{-- BOTÓN DE AUTOANÁLISIS Y TOGGLE --}}
<div
    style="position:fixed;top:300px;left:300px;z-index:10000;display:flex;flex-direction:column;gap:8px;align-items:flex-start;">
    <button id="btn-autoanalyze"
        ...
        mi perfil</button>
    <button id="btn-toggle-overlay"
        style="background:#222;color:#fff;border:none;padding:7px 18px;border-radius:8px;font-weight:700;cursor:pointer;font-size:13px;box-shadow:0 2px 8px rgba(25,25,25,0.12);transition:background 0.2s;">Mostrar/Ocultar
        análisis</button>
</div>
...
@else
    <div id="aoe2-overlay"
        ...
        {{-- STATS GRID --}}
        <div
            style="display:grid; grid-template-columns: repeat(3,1fr); gap:10px; text-align:center; margin-bottom:10px;">
            <div>
        ...
        <div style="display:flex; justify-content: space-between">
            ...
            <div style="margin-bottom:10px;">
                <div style="font-size:11px; color:#aaa; margin-bottom:6px;">UPTIMES ({{ $stats['player_name'] }} vs
                    rivales)
                </div>
                @foreach (['feudal', 'castle', 'imperial'] as $age)
                    <div style="display:flex; justify-content:space-between; padding:2px 0; align-items:center;">
                        ...
                        <div style="display:flex; gap:6px; align-items:center;">
                            <span
                                style="font-weight:700; color:#90caf9;">{{ $stats['avg_' . $age . '_hms'] ?? 'N/A' }}</span>
                ...
                @if (!empty($stats['map_played_percent']))
                    ...
                    @foreach ($stats['map_played_percent'] as $map => $percent)
                        ...
                        <div
                            ...
                            <span
                                style="width:56px; text-align:right; color:#4caf50;">{{ $stats['map_win_percent'][$map] ?? '-' }}%
                                WR</span>
                        </div>
            ...
            @if (!empty($stats['civ_played_percent']))
                <div style="display:flex; flex-wrap:wrap; gap:6px;">
                    @foreach ($stats['civ_played_percent'] as $civ => $percent)
                        <span
                            style="background:rgba(255,255,255,0.06); padding:5px 8px; border-radius:12px; font-size:11px;">
                            {{ $civ }} ({{ $percent }}%)
                        </span>
                    @endforeach
                </div>
        ...
        @php
            ...
            foreach (['feudal', 'castle', 'imperial'] as $age_check) {
                ...
                if (
                    $avgA &&
                    ((isset($avgA['buy']) && count($avgA['buy'])) || (isset($avgA['sell']) && count($avgA['sell'])))
                ) {
                    $market_used = true;
                }
            }
...
<script>
    // Ocultar el overlay tras 15 segundos cuando hay datos
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Toggle para mostrar/ocultar el overlay
    ...
    function create_socket(handler_name) {
        ...
        socket.onmessage = async (event) => {
            ...
            if (!match_data || !match_data.matchId || analyzed_match_ids.has(match_data.matchId) || match_data
            ...
            const new_url =
                `${window.location.pathname}?matchId=${match_data.matchId}&rivalProfileId=${rival_profile_id}&t=${Date.now()}`;
    ...
    function showAnalysis(analysis) {
        ...
        overlay.style =
            'background:#222; color:#fff; padding:12px; margin:12px; white-space:pre; position:fixed; top:50%; right:0; transform:translateY(-50%); z-index:9999; width:700px;';
